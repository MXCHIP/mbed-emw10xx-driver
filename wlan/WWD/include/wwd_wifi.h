 ...
 *            WICED_ERROR   : if an error occurred
 */
extern wwd_result_t wiced_wifi_start_ap( char* ssid, wiced_security_t auth_type, /*@unique@*/ uint8_t* security_key, uint8_t key_length, uint8_t channel );
 ...
 *            WICED_ERROR   : if an error occurred
 */
extern wwd_result_t wiced_wifi_stop_ap( void );
...
extern wiced_result_t wiced_wifi_get_ap_name(char*ssid, int*ssid_len, char*bssid);
...
extern void wiced_get_mac_addr(char *mac);
extern wiced_result_t wiced_wifi_set_maxassoc( uint32_t n );
extern wiced_result_t wiced_wifi_stop_scan(void);
extern wiced_result_t wiced_wifi_set_nmode( int mode );
