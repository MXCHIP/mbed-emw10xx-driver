...
static wiced_country_code_t country_code = WICED_COUNTRY_CHINA;
...
wiced_result_t wiced_init( void )
{
...
#else
    country = country_code;// China support channel 1~13
#endif
    ...
    result = mico_rtos_init_semaphore( &scan_semaphore, 0x7fffffff );
    if ( result != WICED_SUCCESS )
    {
        return result;
    }
    mico_rtos_set_semaphore(&scan_semaphore); /* Semaphore starts at 1 */

#ifndef MXCHIP
    wiced_rtos_init_timer( &wiced_sta_handshake_timer, HANDSHAKE_TIMEOUT_MS, handshake_timeout_handler, 0 );
#endif
    wiced_wifi_set_maxassoc( 4 ); // set SoftAP max support 4 stations.
...
static void link_up( void )
{
    if ( wiced_sta_link_up == WICED_FALSE )
    {
        ...
        mico_rtos_send_asynchronous_event( MICO_WORKER_THREAD, wiced_network_link_up_handler, 0 );
...
static void link_down( void )
{
    if ( wiced_sta_link_up == WICED_TRUE )
    {
        ...
        wiced_rtos_thread_force_awake( &( MICO_WORKER_THREAD->thread ) );

        mico_rtos_send_asynchronous_event( MICO_WORKER_THREAD, wiced_network_link_down_handler, 0 );
...
{
    mico_rtos_send_asynchronous_event( MICO_WORKER_THREAD, handshake_error_callback, 0 );
}
...
static void handshake_timeout_handler( void* arg )
{
    ...
    /* This RTOS timer callback runs in interrupt context. Defer event management to MICO_WORKER_THREAD */
    mico_rtos_send_asynchronous_event( MICO_WORKER_THREAD, handshake_error_callback, 0 );
}
#endif
...
wiced_result_t wiced_wifi_scan_networks( wiced_scan_result_handler_t results_handler, void* user_data )
{
    ...
    result = mico_rtos_get_semaphore( &scan_semaphore, SCAN_LONGEST_WAIT_TIME );
    if ( result != WICED_SUCCESS )
    {
...
static void scan_result_handler( wiced_scan_result_t** result_ptr, void* user_data )
{
    ...
    if ( result_ptr == NULL )
    {
        ...
        {
            if ( mico_rtos_send_asynchronous_event( MICO_WORKER_THREAD, (event_handler_t) scan_handler->results_handler, (void*) ( result_iter ) ) != WICED_SUCCESS )
            {
                free( result_iter  );
            }
        }
        ...
        if ( mico_rtos_send_asynchronous_event( MICO_WORKER_THREAD, (event_handler_t) scan_handler->results_handler, (void*) ( scan_result_ptr ) ) != WICED_SUCCESS )
        {
            free( scan_result_ptr );
        }
...
{
    switch(country_code) {
    case WICED_COUNTRY_CHINA:
        return 13;
    case WICED_COUNTRY_UNITED_STATES:
        return 1;;
    case WICED_COUNTRY_JAPAN:
        return 14;
    default:
        return 13;
    }
...
void set_country_code(wiced_country_code_t code)
{
    country_code = code;
}

void set_country_by_channel(int wifi_channel)
{
    switch(wifi_channel) {
    case WIFI_CHANEEL_1_13://
        country_code = WICED_COUNTRY_CHINA;
        break;
    case WIFI_CHANEEL_1_11://
        country_code = WICED_COUNTRY_UNITED_STATES;
        break;
    case WIFI_CHANEEL_1_14://
        country_code = WICED_COUNTRY_JAPAN;
        break;
    default:
        country_code = WICED_COUNTRY_CHINA;
        break;
