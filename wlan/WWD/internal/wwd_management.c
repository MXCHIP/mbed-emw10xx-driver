 ...
 *             Static Variables
 ******************************************************/
static wiced_mac_t wiced_mac_addr;
#ifdef MICO
extern const int mico_nmode_enable;
#endif
/******************************************************
...
wiced_result_t wiced_management_wifi_on( void )
{
...
#endif
    wiced_wifi_get_mac_address(&wiced_mac_addr);

#ifdef MICO
    if (mico_nmode_enable == 0)
        wiced_wifi_set_nmode(0);
#endif
    ...
    *data = 0;
    ...
    if (( retval != WICED_SUCCESS ) && (retval != WICED_UNSUPPORTED))
    {
    ...
    *ptr = (uint32_t) wiced_wlan_status.country_code & 0x00ffffff;
    ...
    if ( retval != WICED_SUCCESS )
    {
        ...
        WPRINT_WWD_ERROR(("Could not set Country code ret %d\r\n", retval));
        return retval;
    }
    ...
    if ( retval != WICED_SUCCESS )
    {
        ...
        WPRINT_WWD_ERROR(("Could not set Event mask ret %d\r\n", retval));
        return retval;
    }
...
void wiced_get_mac_addr(char *mac)
{
    memcpy(mac, wiced_mac_addr.octet, 6);
}
