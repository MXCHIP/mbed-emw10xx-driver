...
//#define NTOH16(i) htons(i)
//#define NTOH32(i) htonl(i)
//#define HTOL16(i) (i)
//#define HTOL32(i) (i)
//#define ltoh16(i) (i)
//#define ltoh32(i) (i)
...
static uint8_t ioctl_req_sent = 0;
static uint32_t max_bus_credit_diff = CHIP_MAX_BUS_DATA_CREDIT_DIFF;
...
static wiced_bool_t sdpcm_enabled = WICED_FALSE;
...
/** @endcond */
int is_buffer_in_send_queue(wiced_buffer_t buffer);
#ifdef MXCHIPAPP
extern void update_rx_time(void);
#endif
...
void wiced_network_send_ethernet_data( /*@only@*/ wiced_buffer_t buffer, wiced_interface_t interface ) /* Returns immediately - Wiced_buffer_tx_completed will be called once the transmission has finished */
{
    ...
    if (sdpcm_enabled == WICED_FALSE) {
        WPRINT_WWD_DEBUG(("SDPCM is disabled\r\n"));
...
extern void set_max_bus_credit_diff(uint32_t diff);
void set_max_bus_credit_diff(uint32_t diff)
{
    max_bus_credit_diff = diff;
}
...
void wiced_process_sdpcm( /*@only@*/ wiced_buffer_t buffer )
{
    ...
    uint32_t flags;
    uint16_t id;
    sdpcm_common_packet_t*  common_header;
    sdpcm_cdc_header_t*     cdc_header;
    ...
    switch ( packet->sdpcm_header.sw_header.channel_and_flags & 0x0f )
    {
        case CONTROL_HEADER:  /* IOCTL/IOVAR reply packet */
            ...
            common_header = (sdpcm_common_packet_t*) host_buffer_get_current_piece_data_pointer( buffer );
            cdc_header = (sdpcm_cdc_header_t*) &common_header->data[ common_header->sdpcm_header.sw_header.header_length - SDPCM_HEADER_LEN ];
...
wiced_result_t wiced_get_packet_to_send(wiced_buffer_t* buffer)
{
    ...
    if ( wiced_sdpcm_send_queue_head != NULL )
    {
        ...
        {
#ifdef MXCHIP_LIBRARY        
            update_bus_credit_time(WICED_FALSE);
#endif
            return WICED_ERROR;
        }
        sdpcm_line = __LINE__;
...
int is_buffer_in_send_queue(wiced_buffer_t buffer)
{
    wiced_buffer_t p, header;
    
    p = header = wiced_sdpcm_send_queue_head;

    while(p != NULL) {
        if (p == buffer)
            return 1;
        p = wiced_get_next_buffer_in_queue( p );
        if (header != wiced_sdpcm_send_queue_head) {
            p = wiced_sdpcm_send_queue_head;
            header = wiced_sdpcm_send_queue_head;
        }
    }
...
extern int wwd_thread_line;
extern void driver_state_Command( char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv );
...
void driver_state_Command( char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv )
{
    ...
    cmd_printf("available bus credit %d\r\n", wiced_get_available_bus_credits());
    cmd_printf("bus flow control %d\r\n", wiced_bus_is_flow_controlled());
    cmd_printf("wwd thread line %d, sdpcm_line %d\r\n", wwd_thread_line, sdpcm_line);
    p = wiced_sdpcm_send_queue_head;
    if (p == NULL) {
        cmd_printf("Send queue is NULL\r\n");
        return;
    }
    cmd_printf("Send queue: \r\n");
    
    host_rtos_get_semaphore( &wiced_sdpcm_send_queue_mutex, NEVER_TIMEOUT, WICED_FALSE );
    while(p != NULL) {
        p = wiced_get_next_buffer_in_queue( p );
        cmd_printf("    %-3d: Addr 0x%08x, Length %-4d\r\n", i++, 
            host_buffer_get_current_piece_data_pointer(p), 
            host_buffer_get_current_piece_size(p));
    }
    host_rtos_set_semaphore( &wiced_sdpcm_send_queue_mutex, WICED_FALSE );
