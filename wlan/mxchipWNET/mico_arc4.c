...
static void Arc4SetKey(Arc4* arc4, const uint8_t* key, uint32_t length)
{
    uint32_t i;
...
static void Arc4Process(Arc4* arc4, uint8_t* out, const uint8_t* in, uint32_t length)
{
    uint32_t x;
    uint32_t y;
...
void arc4_decrypt(uint8_t*in, uint8_t*out, uint8_t*key, uint32_t keylen, uint32_t in_len)
{
    Arc4 arc4;

    Arc4SetKey(&arc4, key, keylen);
    Arc4Process(&arc4, out, in, in_len);
}
