...
#define DHCP_THREAD_PRIORITY (MICO_DEFAULT_LIBRARY_PRIORITY)
...
wiced_result_t wiced_start_dhcp_server(wiced_dhcp_server_t* server, wiced_interface_t interface)
{
    ...
    /* Create DHCP socket */
    MICO_VERIFY(wiced_udp_create_socket(&server->socket, IPPORT_DHCPS, interface));

    /* Start DHCP server */
    mico_rtos_create_thread(&server->thread, DHCP_THREAD_PRIORITY, "DHCPserver", dhcp_thread, DHCP_STACK_SIZE, (mico_thread_arg_t)server);
...
wiced_result_t wiced_stop_dhcp_server(wiced_dhcp_server_t* server)
{
    ...
    if ( mico_rtos_is_current_thread( &server->thread ) != WICED_SUCCESS )
    {
        mico_rtos_thread_force_awake( &server->thread );
        mico_rtos_thread_join( &server->thread );
        mico_rtos_delete_thread( &server->thread );
    }
    return WICED_SUCCESS;
}
...
static void dhcp_thread( uint32_t thread_input )
{
    ...
    *server_ip_addr_ptr = htobe32( GET_IPV4_ADDRESS( local_ip_address ) );
    ...
    while ( server->quit == WICED_FALSE )
    {
        ...
        if ( wiced_udp_receive( &server->socket, &received_packet, MICO_WAIT_FOREVER ) != WICED_SUCCESS )
        {
            continue;
        }
...
static void ipv4_to_string( char* buffer, uint32_t ipv4_address )
{
    ...
    utoa(ip[0], &buffer[0], 3);
    buffer[3] = '.';
    utoa(ip[1], &buffer[4], 3);
    buffer[7] = '.';
    utoa(ip[2], &buffer[8], 3);
    buffer[11] = '.';
    utoa(ip[3], &buffer[12], 3);
}
