...
static int ap_mode = GMODE_AUTO;
static int ap_hidden = 0;
static int32_t bcnprd = 100;
/******************************************************
 *             Static Function prototypes
 ******************************************************/

static void* wiced_handle_apsta_event( const wiced_event_header_t* event_header, const uint8_t* event_data, /*@returned@*/ void* handler_user_data );

/******************************************************
 *             Function definitions
 ******************************************************/
void wlan_set_ap_mode(int mode);
void wlan_hide_ap(void);
wiced_result_t wlan_wifi_set_bcnprd( int32_t prd );
...
//beacon_interval = 65535; /* No beacons to be sent out */
wiced_result_t wlan_wifi_set_bcnprd( int32_t prd )
{
	if ((prd < 0) || (prd > 65535))
		return WICED_RANGE;
 ...
 */
wwd_result_t wiced_wifi_start_ap( char* ssid, wiced_security_t auth_type, uint8_t* security_key, uint8_t key_length, uint8_t channel )
{
	...
	//wwd_wifi_set_block_ack_window_size(SDPCM_AP_INTERFACE);
    ...
    *data = (uint32_t) WICED_DEFAULT_SOFT_AP_DTIM_PERIOD;
    ...
    data = wiced_get_ioctl_buffer( &buffer, (uint16_t) 4 );
    CHECK_IOCTL_BUFFER_WITH_SEMAPHORE( data, &wiced_wifi_sleep_flag );
    *data = (uint32_t) bcnprd;
    retval = wiced_send_ioctl( SDPCM_SET, WLC_SET_BCNPRD, buffer, 0, SDPCM_AP_INTERFACE );
    wiced_assert("start_ap: Failed to set DTIM\r\n", retval == WICED_SUCCESS );
...
wwd_result_t wiced_wifi_stop_ap( void )
{
    uint32_t* data;
    wiced_buffer_t buffer;
    wiced_buffer_t response;
