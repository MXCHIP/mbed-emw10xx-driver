...
{
    *ip = htonl(IP_HANDLE(interface).ip_addr.addr);
    *mask = htonl(IP_HANDLE(interface).netmask.addr);
    *gw = htonl(IP_HANDLE(interface).gw.addr);
    return WICED_SUCCESS;
}
...
#else
    handler->pcb.udp->if_index = (interface + 1);
#endif
    /* send a packet */
    packet->p->len = packet->p->tot_len;
