...
#ifndef GKI_DEBUG_BUF
#define GKI_DEBUG_BUF    FALSE
#endif
/*******************************************************************************
**
...
static void gki_init_free_queue (UINT8 id, UINT16 size, UINT16 total, void *p_mem)
{
    ...
    /* Check for unused pool */
    if (total == 0)
    {
        p_cb->pool_start[id] = p_cb->pool_end[id] = NULL;
        memset(&p_cb->freeq[id], 0, sizeof(FREE_QUEUE_T));
        return;
    }
...
**
*******************************************************************************/
#if (!defined(GKI_DYNAMIC_POOL_CFG) || (GKI_DYNAMIC_POOL_CFG == FALSE)) /* Initialize buffer pools for static GKI buffer configuration */
...
#else   /* Initialize buffer pools for dynamic GKI buffer configuration */
void gki_buffer_init(void)
{
    UINT8   tt, mb;
    tGKI_COM_CB *p_cb = &gki_cb.com;
    ...
    p_cb->freeq = (FREE_QUEUE_T *)GKI_os_malloc(MICO_BT_CFG_NUM_BUF_POOLS * sizeof(FREE_QUEUE_T));
    p_cb->pool_start = (UINT8 **)GKI_os_malloc(MICO_BT_CFG_NUM_BUF_POOLS * sizeof(UINT8 *));
    p_cb->pool_end = (UINT8 **)GKI_os_malloc(MICO_BT_CFG_NUM_BUF_POOLS * sizeof(UINT8 *));
    p_cb->pool_size = (UINT16 *)GKI_os_malloc(MICO_BT_CFG_NUM_BUF_POOLS * sizeof(UINT16));
    p_cb->pool_list = (UINT8 *)GKI_os_malloc(MICO_BT_CFG_NUM_BUF_POOLS * sizeof(UINT8));

    if ((!p_cb->freeq) || (!p_cb->pool_start) || (!p_cb->pool_end) ||
        (!p_cb->pool_size) || (!p_cb->pool_list))
    {
        /* Error allocating buffer pools */
        GKI_exception(GKI_ERROR_OUT_OF_BUFFERS, "Unable to allocate buffer pools");
        return;
    }

    for (tt = 0; tt < GKI_NUM_TOTAL_BUF_POOLS; tt++)
    {
        /* Allocate buffer pool (reserve space in each buffer for GKI buffer header) */
        if (p_btm_cfg_buf_pools[tt].buf_count)
        {
            p_mem = (void *)GKI_os_malloc(p_btm_cfg_buf_pools[tt].buf_count *
                (p_btm_cfg_buf_pools[tt].buf_size + BUFFER_PADDING_SIZE));
        }

        /* Inialize buffer headers and link the buffers into freeq*/
        gki_init_free_queue(tt, 
            p_btm_cfg_buf_pools[tt].buf_size, 
            p_btm_cfg_buf_pools[tt].buf_count, 
            p_mem);

        p_cb->pool_list[tt] = tt;
    }
...
void *GKI_getbuf (UINT16 size)
{
    ...
    for ( ; i < p_cb->curr_total_no_of_pools; i++)
    {
        ...
        if ((Q->cur_cnt < Q->total) && (Q->p_first))
        {
            ...
            p_hdr->Type    = 0;
#if (GKI_DEBUG_BUF == TRUE)
            GKI_TRACE_5("GKI_getbuf(size: %d/%d) 0x%x task:%d cur_cnt:%d", Q->size, size,
                ((UINT8 *)p_hdr + BUFFER_HDR_SIZE), p_hdr->task_id, cur_cnt);
#endif
...
void *GKI_getpoolbuf (UINT8 pool_id)
{
    ...
    if ((Q->cur_cnt < Q->total) && (Q->p_first))
    {
        ...
        p_hdr->Type    = 0;
#if (GKI_DEBUG_BUF == TRUE)
        GKI_TRACE_3("GKI_getpoolbuf(size: %d) 0x%x task:%d", Q->size,
            ((UINT8 *)p_hdr + BUFFER_HDR_SIZE), p_hdr->task_id);
#endif
        return ((void *) ((UINT8 *)p_hdr + BUFFER_HDR_SIZE));
    }
...
void GKI_freebuf (void *p_buf)
{
    ...
    {
#if (GKI_DEBUG_BUF == TRUE)
        GKI_TRACE_2("GKI_freebuf 0x%x task:%d", p_buf, GKI_get_taskid());
#endif
    ...
    {
#if (GKI_DEBUG_BUF == TRUE)
        GKI_TRACE_3("GKI_freebuf 0x%x (exception) task:%d/%d", p_buf, p_hdr->task_id, GKI_get_taskid());
#endif
    ...
    {
#if (GKI_DEBUG_BUF == TRUE)
        GKI_TRACE_3("GKI_freebuf 0x%x task:%d/%d", p_buf, p_hdr->task_id, GKI_get_taskid());
#endif
