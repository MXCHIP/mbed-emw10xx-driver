...
#define BTM_SEC_NVRAM_BONDED_DEVICE_ENTRY_MAX_LEN  (sizeof(mico_bt_nvram_access_entry_t) - 1 + BTM_SEC_NVRAM_KEYBLOB_MAX_LEN)
#endif


/* Default nvram access functions for saving/retrieving link keys */
extern void mico_bt_nvram_access_init(void);
extern mico_bt_result_t mico_bt_nvram_access_get_bonded_devices(mico_bt_dev_bonded_device_info_t paired_device_list[], uint16_t *p_num_devices);
extern mico_bt_result_t mico_bt_nvram_access_save_bonded_device_key(mico_bt_device_address_t bd_addr, mico_bt_ble_address_type_t addr_type, uint8_t device_type, uint8_t *p_keyblobs, uint16_t key_len);
extern mico_bt_result_t mico_bt_nvram_access_load_bonded_device_keys(mico_bt_device_address_t bd_addr, mico_bt_nvram_access_entry_t *p_key_entry, uint8_t entry_max_length);
extern mico_bt_result_t mico_bt_nvram_access_delete_bonded_device(mico_bt_device_address_t bd_addr);
extern mico_bt_result_t mico_bt_nvram_access_load_local_identity_keys(mico_bt_local_identity_keys_t *p_lkeys);
extern mico_bt_result_t mico_bt_nvram_access_save_local_identity_keys(mico_bt_local_identity_keys_t *p_lkeys);
extern mico_bool_t mico_bt_nvram_access_key_storage_available(mico_bt_device_address_t bd_addr,int req_size);
extern mico_bt_result_t mico_bt_nvram_access_enum_bonded_device_keys(int8_t *p_index, mico_bt_nvram_access_entry_t * p_key_entry, uint8_t entry_max_length);

extern mico_bt_nvram_access_t              *p_btm_nvram_access;     /* current nvram access functions */
