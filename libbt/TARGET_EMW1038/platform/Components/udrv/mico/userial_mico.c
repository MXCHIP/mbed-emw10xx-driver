...
int bt_bus_init( void )
{
    ...
    if ( bus_initialised == false )
    {
        ...
        require_noerr( MicoUartInitialize( BT_UART, &mico_bt_uart_config, &rx_ring_buffer ), exit );
...
int bt_bus_deinit( void )
{
    ...
    /* Deinitialise UART */
    require_noerr( MicoUartFinalize( BT_UART ), exit );
...
int bt_bus_transmit( const uint8_t* data_out, uint32_t size )
{
    ...
    require_noerr( MicoUartSend( BT_UART, data_out, size ), exit );
...
int bt_bus_uart_reconifig_baud(uint32_t baud)
{
    ...
    if ( bus_initialised == true )
    {
        ...
        platform_uart_reconfig( mico_bt_uart_driver, &uart_config );
        return kNoErr;
    }
    else
    {
