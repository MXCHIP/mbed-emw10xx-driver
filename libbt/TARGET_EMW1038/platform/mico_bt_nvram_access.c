...
static mico_bool_t mico_bt_nvram_access_valid_entry(mico_bt_nvram_access_entry_t *p_entry)
{
    ...
    APPL_TRACE_DEBUG3 ("mico_bt_nvram_access_valid_entry  BDA: %08x%04x, key length: %d",
                       (p_entry->bd_addr[0] << 24) + (p_entry->bd_addr[1] << 16) + (p_entry->bd_addr[2] << 8) +
                       p_entry->bd_addr[3],
...
void mico_bt_nvram_access_init()
{
    ...
    */
    ...
    do {
        mico_system_para_read((void **) &p_entry, PARA_BT_DATA_SECTION, dct_cb.cur_offset,
                              MICO_BT_DCT_ENTRY_HDR_LENGTH);
    ...
    APPL_TRACE_DEBUG4("%s start offset %d %d, end_offset: %d", __FUNCTION__, dct_cb.start_offset, dct_size,
                      dct_cb.end_offset);
}
 ...
 */
static mico_bool_t
mico_bt_nvram_access_find_offset(mico_bt_device_address_t key_bdaddr, int start_offset, int *p_offset)
{
    ...
    do {
        mico_system_para_read((void **) &p_entry,
                              ...
                              of_dct,
                              MICO_BT_DCT_ADDR_FIELD + MICO_BT_DCT_LENGTH_FIELD + MICO_BT_DCT_ADDR_TYPE +
                              MICO_BT_DCT_DEVICE_TYPE);
 ...
 */
mico_bt_result_t
mico_bt_nvram_access_get_bonded_devices(mico_bt_dev_bonded_device_info_t bonded_device_list[], uint16_t *p_num_devices)
{
 ...
 */
mico_bt_result_t
mico_bt_nvram_access_save_bonded_device_key(mico_bt_device_address_t bd_addr, mico_bt_ble_address_type_t addr_type,
                                            uint8_t device_type, uint8_t *p_keyblobs, uint16_t key_len)
{
    int of_key = 0;
    ...
    if (mico_bt_nvram_access_find_offset(bd_addr, dct_cb.start_offset, &of_key)) {
        ...
        if (p_entry->length != key_len) {
            ...
            if (mico_bt_nvram_access_valid_entry(p_next_entry)) {
                ...
                mico_system_para_read((void **) &p_updated, PARA_BT_DATA_SECTION,
                                      of_key + p_entry->length + MICO_BT_DCT_ENTRY_HDR_LENGTH, len_updated);
                /* dct_write of current key entry with changed length */
                mico_system_para_write(p_updated, PARA_BT_DATA_SECTION, of_key + key_len + MICO_BT_DCT_ENTRY_HDR_LENGTH,
                                       len_updated);
            ...
            mico_system_para_write(&addr_type, PARA_BT_DATA_SECTION,
                                   of_key + MICO_BT_DCT_ADDR_FIELD + MICO_BT_DCT_LENGTH_FIELD,
                                   sizeof(mico_bt_ble_address_type_t));
            mico_system_para_write(&device_type, PARA_BT_DATA_SECTION,
                                   of_key + MICO_BT_DCT_ADDR_FIELD + MICO_BT_DCT_LENGTH_FIELD + MICO_BT_DCT_ADDR_TYPE,
                                   MICO_BT_DCT_DEVICE_TYPE);
            ...
            if (memcmp(p_entry->key_blobs, p_keyblobs, key_len)) {
                mico_system_para_write(p_keyblobs, PARA_BT_DATA_SECTION, of_key + MICO_BT_DCT_ENTRY_HDR_LENGTH,
                                       key_len);
            } else {
...
mico_bt_result_t mico_bt_nvram_access_load_bonded_device_keys(mico_bt_device_address_t bd_addr,
                                                              mico_bt_nvram_access_entry_t *p_key_entry,
                                                              uint8_t entry_max_length)
{
    ...
    if (mico_bt_nvram_access_find_offset(bd_addr, dct_cb.start_offset, &of_key)) {
        ...
        mico_system_para_read_release((void *) p_entry);
        mico_system_para_read((void **) &p_keyblobs, PARA_BT_DATA_SECTION, of_key + MICO_BT_DCT_ENTRY_HDR_LENGTH,
                              p_entry->length);
 ...
 * @return      TRUE if there is available space or FALSE
 */
mico_bt_result_t mico_bt_callout_get_extra(mico_bt_device_address_t bd_addr, mico_bt_ble_address_type_t *p_addr_type,
 ...
 */
mico_bt_result_t
mico_bt_nvram_access_enum_bonded_device_keys(int8_t *p_index, mico_bt_nvram_access_entry_t *p_key_entry,
                                             uint8_t entry_max_length)
{
    mico_bt_nvram_access_entry_t *p_entry;
    uint8_t *p_key_blobs;
    ...
    mico_system_para_read_release((void *) p_entry);
    mico_system_para_read((void **) &p_key_blobs, PARA_BT_DATA_SECTION,
                          dct_cb.cur_offset + MICO_BT_DCT_ENTRY_HDR_LENGTH, p_entry->length);
...
mico_bool_t mico_bt_nvram_access_find_device(mico_bt_device_address_t key_bdaddr)
{
    ...
    do {
        mico_system_para_read((void **) &p_entry, PARA_BT_DATA_SECTION, of_dct,
                              MICO_BT_DCT_ADDR_FIELD + MICO_BT_DCT_LENGTH_FIELD + MICO_BT_DCT_ADDR_TYPE +
                              MICO_BT_DCT_DEVICE_TYPE);
        if (!memcmp(p_entry->bd_addr, key_bdaddr, sizeof(mico_bt_device_address_t))) {
            APPL_TRACE_DEBUG2("%s found keyblobs: %d ", __FUNCTION__, of_dct);
